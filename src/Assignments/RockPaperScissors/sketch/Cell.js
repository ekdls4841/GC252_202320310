class Cell {
  ...
  constructor(x, y, w, h, state = random(["R", "P", "S"])) {
    this.pos = [x, y];
    this.size = [w, h];
    this.state = state;
  }
  ...
  computeNextState() {
    const alive = this.neighbors.filter((n) => n);
    if (alive.length === 0) {
      this.nextState = this.state;
      return;
    }
    const picked = random(alive);
    const beats = { R: "S", P: "R", S: "P" };
    if (beats[picked.state] === this.state) {
      this.nextState = picked.state;
    } else {
      this.nextState = this.state;
    }
  }
